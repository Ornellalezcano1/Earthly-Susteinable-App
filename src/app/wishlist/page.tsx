            ...
            <div className="lg:col-span-7 space-y-12">
              <div className="space-y-6">
                ...
                <p className="text-white/40 text-lg font-light leading-relaxed tracking-tight">
                  This selection is waiting for your next adventure. Our curated sustainable plan ensures you experience the best of {item.location} while protecting its natural beauty and supporting local communities.
                </p>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 pt-8 border-t border-white/5">
                <div className="space-y-6">
                  <h4 className="text-lg font-medium text-white uppercase tracking-tighter">Availability</h4>
                  <p className="text-base font-light italic text-white/50">Seasonal bookings open for this year. Limited spots available.</p>
                </div>
                <div className="space-y-6">
                  <h4 className="text-lg font-medium text-white uppercase tracking-tighter">Status</h4>
                  <div className="flex items-center gap-2 text-blue-400">
                    <Heart size={18} fill="currentColor" />
                    <span className="text-base font-medium">Saved in Wishlist</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-5 space-y-8">
              <div className="bg-white/[0.02] border border-white/10 rounded-[32px] p-6 md:p-8 space-y-6 shadow-inner">
                <div className="flex items-center justify-between border-b border-white/5 pb-5">
                  ...
                  <span className="text-lg font-medium tracking-tight text-white">{item.rating}</span>
                </div>

                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-2xl bg-emerald-400/10 flex items-center justify-center text-emerald-400">
                      <CreditCard size={20} />
                    </div>
                    <span className="text-xs font-medium text-white/40 uppercase tracking-widest">Price</span>
                  </div>
                  <span className="text-xl font-medium text-white tracking-tighter">${item.price}</span>
                </div>
              </div>

              <div className="space-y-4">
                <button className="w-full bg-white text-black py-4 rounded-[18px] font-bold text-xs tracking-[0.1em] uppercase transition-all hover:bg-white/90 hover:scale-[1.01] active:scale-[0.98] flex items-center justify-center gap-2 group">
                  Confirm Booking
                  <ChevronRight size={14} className="group-hover:translate-x-0.5 transition-transform" />
                </button>
                <button 
                  onClick={() => { onRemove(item.id); onClose(); }}
                  className="w-full bg-transparent text-white/40 py-2 rounded-[18px] font-medium text-[10px] tracking-[0.1em] uppercase transition-all hover:text-red-400 flex items-center justify-center gap-2"
                >
                  <Trash2 size={12} />
...
export default function App() {
  ...
  // Dynamic state for wishlist items with corrected local images
  ...
  return (
    <main className="relative w-full h-screen bg-[#050609] font-sans text-white overflow-x-hidden overflow-y-auto 
      ...
      <div className="relative z-10 flex flex-col w-full min-h-full pb-[60px]">
        ...
        <div className="px-[30px] flex flex-col lg:flex-row gap-6 mb-12 items-center justify-between">
          <div className="flex flex-wrap gap-3 w-full lg:w-auto">
            {filterCategories.map((cat) => (
              <button 
                key={cat}
                onClick={() => setSelectedCategory(cat)}
                className={`px-8 py-3 rounded-full border border-white/10 text-sm transition-all duration-300
          ...
          <div className="flex items-center gap-4 w-full lg:w-auto justify-end">
             <button 
               ...
               className={`flex items-center gap-2 px-6 py-3 rounded-2xl border transition-all text-sm font-medium
                ${isAdvancedFilterOpen ? 'bg-blue-500 border-blue-400 text-white' : 'bg-white/5 border-white/10 text-white/70 hover:bg-white/10'}`}
             >
                <SlidersHorizontal size={18} />
                {isAdvancedFilterOpen ? 'Active Filters' : 'Advanced Filter'}
             </button>
          </div>
        </div>

        {isAdvancedFilterOpen && (
          <div className="mx-[30px] mb-12 p-8 bg-white/[0.03] border border-white/10 rounded-[35px] animate-in slide-in-from-top-4 duration-500">
             <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div>
                  <div className="flex justify-between items-center mb-3">
                    <label className="block text-xs uppercase tracking-widest text-white/40">Max Price</label>
                    <span className="text-sm font-medium text-white">${maxPrice}</span>
                  </div>
                  <input 
                    type="range" 
                    min="0" 
                ...
                <div>
                  <label className="block text-xs uppercase tracking-widest text-white/40 mb-3">Sort by</label>
                  <select 
                    value={sortBy}
                    onChange={(e) => setSortBy(e.target.value)}
                    className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none appearance-none cursor-pointer"
                  >
          ...
          {filteredItems.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10">
              {filteredItems.map((item) => (
                <div
                  key={item.id}
                  className="group relative flex flex-col bg-white/5 rounded-[35px] border border-white/10 overflow-hidden transition-all duration-500 hover:bg-white/[0.08] hover:-translate-y-2 shadow-2xl"
                >
                  ...
                  <div className="p-8 flex flex-col flex-1">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <div className="w-14 h-14 rounded-full bg-white/10 backdrop-blur-md border border-white/20 flex items-center justify-center shrink-0 shadow-xl group-hover:bg-white/20 transition-all duration-300">
                        <span className="text-white font-semibold text-sm">${item.price}</span>
                      </div>
                    </div>
                    <p className="text-white/50 text-sm font-light leading-relaxed mb-8 line-clamp-2">{item.description}</p>
                    <button 
            ...
            <div className="flex flex-col items-center justify-center py-32 text-center space-y-6">
              ...
              <div>
                ...
                <p className="text-white/40 font-light mt-2">Try adjusting your filters to see more options.</p>
                <button 
                  onClick={resetAllFilters}
                  className="mt-6 text-white underline underline-offset-4 hover:text-blue-400"
                >
                  Clear filters
                </button>
              </div>
